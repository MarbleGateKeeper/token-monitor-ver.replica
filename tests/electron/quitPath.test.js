...
test('before-quit hands over without taking the quit away from the OS', () => {
  ...
  // This fork has no installer hand-off, so every quit route must reach the
  // same synchronous exit without an updater-only bypass.
  assert.doesNotMatch(handler, /skipForcedQuit/);
  assert.doesNotMatch(main, /electron-updater|quitAndInstall|skipForcedQuit/);
});
