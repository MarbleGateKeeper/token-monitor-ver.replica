...
test('publishes projected stats to the macOS Widget on collection and presentation changes', () => {
  ...
  assert.match(
    sendPush,
    /const stats = mappedStatsForDisplay\(injectLocalDeviceStatus\(payload\.data\.stats\)\);\s+acceptedStats = stats;\s+latestStats = stats;\s+const visibleStats = electronPresentationStats\(latestStats\);/
  );
...
test('starts the runtime after pricing initialization and holds only Widget publication until host registration settles', () => {
  const readyStart = mainSource.indexOf('app.whenReady().then(async () => {');
  const readyEnd = mainSource.indexOf("ipcMain.handle('settings:get'", readyStart);
