...
- Release discovery and repository/about links belong to this fork. `src/shared/appUpdater.js` accepts only `*-replica.N` tags in the fork update channel and checks upstream separately for a maintenance reminder; a plain upstream version must never outrank the same-base replica build. Both are notification-only. The tag workflow publishes notes without project-built binaries or updater metadata; do not reintroduce download/install behavior while syncing upstream.
...
When syncing upstream, fetch and review `upstream/main`, preserve the fork-specific behavior documented in README.md while resolving conflicts, then audit the diff for newly added repository/update URLs. Set the replica version only after the upstream merge is complete, keep the original license in both source and locally built packaged resources, update README.md plus release notes, and run `npm run verify`. Build a target-platform package only when local validation needs it; never attach it to this fork's automated Release.
...
| Docs & env examples | the bilingual supported-tools table in `README.md` + the client CSV in `.env.example`. Its Chinese and English prose counts must match the table — `tests/docs/readmeConsistency.test.js` fails on a stale count or a translated README copy being reintroduced |
