...
- Release discovery and repository/about links belong to this fork. `src/shared/appUpdater.js` checks this repository's latest Release for notification purposes only. The tag workflow publishes notes without project-built binaries or updater metadata; do not reintroduce download/install behavior while syncing upstream.
